...
export default function App() {
      ...
      if(link) {
        ...
        sleep(2000).then(() => {
          ...
          if(webview.current) {
            ...
            /*
            fetch('https://m.brandkorea.info/common/pushLinkMove?link='+link+'&os='+Platform.OS, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              }
            }).catch((err) => console.error('err cn: ', err));  
            */
          } else {
            let enLink = encodeURIComponenet(link);
            console.error('fetch called: ', 'https://m.brandkorea.info/common/pushLinkMove?link='+enLink+'&os='+Platform.OS);
            fetch('https://m.brandkorea.info/common/pushLinkMove?link='+enLink+'&os='+Platform.OS, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              }
  ...
  async function registerForPushNotificationsAsync() {
    ...
    if (Device.isDevice) {
      ...
      token = (await Notifications.getExpoPushTokenAsync({ projectId: 'd13cd83a-e231-48c3-af78-7bbb323b3224' }));
      ...
      if(token && token.data) {
        ...
        fetch('https://m.brandkorea.info/common/putToken?token='+rtv+'&os='+Platform.OS, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              }
